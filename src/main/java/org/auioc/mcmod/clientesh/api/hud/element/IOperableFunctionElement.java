...
public interface IOperableFunctionElement extends IFunctionElement, IOperableValue {
    ...
    default IHudElement getResult() {
        ...
        if (r instanceof IStringValue v) return new StringElement(v.stringValue());
        if (r instanceof IIntegerValue v) return new IntegerElement(v.intValue());
        if (r instanceof IDoubleValue v) return new DoubleElement(v.doubleValue());
        if (r instanceof IBooleanValue v) return new BooleanElement(v.booleanValue());
